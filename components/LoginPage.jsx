...
export default function LoginPage({ supabase, onAuth }) {
  ...
  const S = {
    ...
    input: { width: '100%', padding: '12px 14px', background: BRAND.bg, border: '1px solid ' + BRAND.border, borderRadius: 8, color: '#fff', fontSize: 14, fontFamily: 'Sora, sans-serif', outline: 'none', boxSizing: 'border-box' },
    btn: { width: '100%', padding: '13px 0', background: BRAND.accent, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'Sora, sans-serif', marginTop: 8, opacity: loading ? 0.7 : 1 },
  ...
  return (
    <div style={S.page}>
      <div style={S.container}>
        <div style={S.logo}>
          <div style={S.logoText}><span style={S.logoAccent}>K</span>apturise</div>
          <div style={S.subtitle}>
            {mode === 'login' ? 'Sign in to your AI Agents dashboard' :
             mode === 'signup' ? 'Create your account to get started' :
             'Reset your password'}
          </div>
          <div style={S.agents}>{Array.from({length: 17}, (_, i) => (<div key={i} style={S.agentDot(i)} />))}</div>
        </div>
        ...
        {mode === 'login' && (
          <form style={S.form} onSubmit={handleLogin}>
            <div><label style={S.label}>Email</label><input style={S.input} type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@company.com" required autoFocus /></div>
            <div><label style={S.label}>Password</label><input style={S.input} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" required minLength={6} /></div>
            <button style={S.btn} type="submit" disabled={loading}>{loading ? 'Signing in...' : 'Sign In'}</button>
            <div style={{ textAlign: 'right', marginTop: -8 }}><button type="button" style={S.link} onClick={() => { setMode('forgot'); setError(''); setMessage(''); }}>Forgot password?</button></div>
            <div style={S.divider}><div style={S.line} /><span>or</span><div style={S.line} /></div>
            <div style={S.toggle}>Don&apos;t have an account? <button type="button" style={S.link} onClick={() => { setMode('signup'); setError(''); setMessage(''); }}>Sign up</button></div>
          </form>
        )}
        ...
        {mode === 'signup' && (
          <form style={S.form} onSubmit={handleSignup}>
            <div><label style={S.label}>Email</label><input style={S.input} type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@company.com" required autoFocus /></div>
            <div><label style={S.label}>Password</label><input style={S.input} type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Min 6 characters" required minLength={6} /></div>
            <div><label style={S.label}>Confirm Password</label><input style={S.input} type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="••••••••" required minLength={6} /></div>
            <button style={S.btn} type="submit" disabled={loading}>{loading ? 'Creating account...' : 'Create Account'}</button>
            <div style={S.divider}><div style={S.line} /><span>or</span><div style={S.line} /></div>
            <div style={S.toggle}>Already have an account? <button type="button" style={S.link} onClick={() => { setMode('login'); setError(''); setMessage(''); }}>Sign in</button></div>
          </form>
        )}
        ...
        {mode === 'forgot' && (
          <form style={S.form} onSubmit={handleForgot}>
            ...
            <div style={S.toggle}><button type="button" style={S.link} onClick={() => { setMode('login'); setError(''); setMessage(''); }}>Back to sign in</button></div>
          </form>
        )}
