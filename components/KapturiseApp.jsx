...
If a prospect replies on any channel, continue the conversation there. If they give a phone number → schedule a call. If they ask for pricing → generate a proposal.`}</div>
  ...
  function AgentWork({agent,agents,setAgents,scripts,isApproached,markApproached,scoreProspect,autoCreateFollowUp}){
    const[tab,setTab]=useState("workflow");const[result,_sr]=useState(_AR[agent.id]||null);const setResult=v=>{const nv=typeof v==="function"?v(result):v;_AR[agent.id]=nv;_sr(nv);};const[loading,setLoading]=useState(false);const[custom,setCustom]=useState("");const ref=useRef(null);
