...
if (!supabaseUrl || !supabaseAnonKey) {
  console.warn('Supabase not configured â falling back to localStorage');
}
  ...
  async get(key) {
    if (supabase) {
      ...
      if (table) {
        ...
        if (!error && data && data.length > 0) return { key, value: JSON.stringify(data) };
      }
    }
    try {
...
function keyToTable(key) {
  const map = {
    'k8-ag': 'agents', 'k10-ld': 'leads', 'k10-inv': 'investors',
    'k7-pr': 'pricing', 'k7-cm': 'campaigns', 'k7-scripts': null,
    'k8-so2': 'social_accounts', 'k7-co': 'company_info', 'k7-ak': 'api_keys',
  };
  return map[key] || null;
}
