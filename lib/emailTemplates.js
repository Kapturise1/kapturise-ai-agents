...
function makeCTA(ctaText, ctaSubject) {
  const waNumber = '971559137354';
  const waMessage = encodeURIComponent(`Hi Kapturise! I'm interested in ${ctaSubject}. Can we chat?`);
  return `<div style="background:#1a1a2e;color:#fff;padding:16px;border-radius:8px;margin:20px 0;text-align:center;">
    ...
    <p style="margin:0;">Let's have a quick chat — reach out to us on WhatsApp anytime.</p>
    <p style="margin:12px 0 0 0;">
      <a href="https://wa.me/${waNumber}?text=${waMessage}" style="background:#25D366;color:#fff;padding:10px 24px;text-decoration:none;border-radius:5px;font-weight:bold;">&#128172; Connect on WhatsApp &#8594;</a>
    </p>
  </div>`;
}
...
function makeAIVideoRetainerTable() {
  return `<p style="margin-top:24px;"><strong>&#129302; AI Video Monthly Retainer — 6 Month Commitment</strong></p>
  <table style="${tableStyle}">
    ${monthlyHeaderRow}
    ${row('&#129302; AI Video Retainer — Standard', '15 AI-generated videos/month (30-60s each), professional scripts, motion graphics, royalty free music, brand-consistent style, monthly content calendar', 'AED 20,000/month', true)}
    ${row('&#129302; AI Video Retainer — Premium', '30 AI-generated videos/month (30-60s each), professional scripts, motion graphics, royalty free music, brand-consistent style, monthly content calendar, priority turnaround', 'AED 35,000/month', false)}
  </table>
  <p style="color:#888;font-size:13px;"><em>* 6-month retainer commitment. Prices exclude 5% VAT. Includes AI product videos, AI hero videos, UGC-style AI videos, social media cuts.</em></p>`;
}
...
export const EMAIL_TEMPLATES = {
  ...
  <table style="${tableStyle}">
    ...
    ${row('&#129302; AI & UGC Video Production', 'AI videos, UGC-style videos, AI product videos, AI hero videos with real people (30s–60s), professional script, motion graphics, royalty free music', 'AED 1,500 + VAT', true)}
  </table>
  ${startingNote}
  ${makeAddOns(['Food Stylist', 'Models', 'Influencers', 'Props', 'Studio Spaces', 'Retouching', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  ${startingNote}
  ${makeAddOns(['Real Estate Staging', 'Drone Shoot', '360° VR Virtual Tour', 'Retouching', 'Models', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  <table style="${tableStyle}">
    ...
    ${row('&#129302; AI & UGC Video Production', 'AI videos, UGC-style videos, AI product videos, AI hero videos with real people (30s–60s), professional script, motion graphics, royalty free music', 'AED 1,500 + VAT', false)}
  </table>
  ${startingNote}
  ${makeAddOns(['Extra Photographer', 'Extra Videographer', 'Live Streaming', 'Retouching', 'Models', 'Influencers / MC / Hosts', 'Stage Build', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  <table style="${tableStyle}">
    ${headerRow}
    ${row('&#128084; Corporate Photography', '40 edited images (enhanced), lifestyle & corporate headshots, professional camera with lenses', 'AED 1,600 + VAT', true)}
    ${row('&#127916; Corporate Videography', 'Script writing, TVC corporate video (1 min), royalty free music, professional camera with gimbal, wireless mic', 'AED 3,000 + VAT', false)}
    ${row('&#127912; Product Photography', '20 SKUs, 3 angles per SKU, 4-hour shoot, edited images (enhanced)', 'AED 3,000 + VAT', true)}
    ${row('&#129302; AI & UGC Video Production', 'AI videos, UGC-style videos, AI product videos, AI hero videos with real people (30s–60s), professional script, motion graphics, royalty free music', 'AED 1,500 + VAT', false)}
  </table>
  ${startingNote}
  ${makeAddOns(['Retouching', 'Models', 'Influencers / MC / Hosts', 'Live Streaming', 'Stage Build', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  ${startingNote}
  ${makeAddOns(['Retouching', 'Models', 'Influencers', 'Lifestyle Shots', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  <table style="${tableStyle}">
    ...
    ${row('&#127910; Event / Venue Coverage', '1 photographer + 1 videographer, edited highlight video (60s), edited images', 'AED 3,000 + VAT', true)}
    ${row('&#129302; AI & UGC Video Production', 'AI videos, UGC-style videos, AI product videos, AI hero videos with real people (30s–60s), professional script, motion graphics, royalty free music', 'AED 1,500 + VAT', false)}
  </table>
  ${startingNote}
  ${makeAddOns(['Drone Shoot', '360° VR Virtual Tour', 'Real Estate Staging', 'Retouching', 'Food Stylist', 'Models', 'Influencers', 'Voice Over'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
  ...
  <table style="${tableStyle}">
    ...
    ${row('&#128084; Corporate Photography', '40 edited images (enhanced), lifestyle & headshots, professional camera with lenses', 'AED 1,600 + VAT', false)}
    ${row('&#127912; Product Photography', '20 SKUs, 3 angles each, 4-hour shoot, edited images (enhanced)', 'AED 3,000 + VAT', true)}
    ${row('&#127916; Videography', 'Edited video, royalty free music, professional camera with gimbal, wireless mic', 'AED 3,000 + VAT', false)}
    ${row('&#129302; AI & UGC Video Production', 'AI videos, UGC-style videos, AI product videos, AI hero videos with real people (30s–60s), professional script, motion graphics, royalty free music', 'AED 1,500 + VAT', true)}
  </table>
  ${startingNote}
  ${makeAddOns(['Models', 'Food Stylist', 'Influencers', 'Retouching', 'Voice Over', 'Drone Coverage', '360° VR Virtual Tour'])}
  ${makeAIVideoRetainerTable()}
  ${makeMonthlyTable()}
